...
/**
 * Helper function to generate success HTML for popup responses
 */
...
/**
 * Helper function to generate error HTML for popup responses
 */
...
/**
 * Helper function to check if the request is from a popup
 */
function isPopupRequest(req: Request): boolean {
  // First try to extract popup flag from state if available
  ...
  // Check user agent if headers exist
  if (req.headers && req.headers["user-agent"]) {
    return req.headers["user-agent"].toString().includes("gmailAuthPopup");
  }
...
export async function gmailCallback(req: Request, res: Response) {
  try {
    ...
    if (isPopup) {
      ...
      res.send(generateErrorHtml(message));
    } else {
      // Traditional redirect for non-popup flows
      res.redirect(
        `${
